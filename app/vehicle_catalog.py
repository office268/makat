...
def popular_models(make=None, limit=5):
    ...
    אין במאגר הזה נתוני מכירות, אבל לדגם נפוץ יש יותר קודי דגם, רמות
    גימור ומנועים רשומים. זה היה הפרוקסי היחיד לפופולריות עד שנוספה
    ספירת הצי (app/fleet_stats.py), והוא נשאר כרשת ביטחון: מנוע הגילוי
    מכוון לפי דירוג הפערים, ונופל לכאן רק כשעוד לא נספר צילום.
    """
