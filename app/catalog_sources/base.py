...
class Candidate:
    ...
    diagram_url: str = ""
    # המחיר כפי שהוא מופיע בעמוד, ולצידו המטבע שלו. שני שדות ולא אחד,
    # כי המקורות אינם באותו מטבע: קטלוג ישראלי מתמחר בשקלים, קטלוג
    # אירופי ביורו וקטלוג בינלאומי בדולר. שדה בשם ``price_eur`` - כפי
    # שהיה כאן - הפך כל מחיר שאינו יורו לשקר שקט, בלי שום מקום שבו
    # הוא נחשף. מטבע ריק פירושו "המקור לא אמר", ולא "יורו".
    price_listed: float = None
    currency: str = ""
    source_url: str = ""
    source_key: str = ""
    variant_key: str = ""
...
@dataclass
class Continuation:
    """איפה להמשיך, כששליפה לא הספיקה בבקשת HTTP אחת.

    מקור שמנווט בקטלוג עושה כמה הבאות ברצף: עמוד הרכב -> קבוצה ->
    תרשים. כל אחת מהן היא עשרות שניות דרך ScraperAPI, ו-gunicorn הורג
    בקשה אחרי ``WEB_TIMEOUT``. שלוש הבאות בבקשה אחת לא נכנסות בתקציב,
    ומורידים אותן לשתיים - כפי שהיה - פשוט לא מגיעים לתרשים.

    ‏``live_lookup`` כבר בנוי סביב "צעד אחד לכל בקשה"; זה מה שמאפשר
    להחיל את אותו כלל גם על הצעדים *בתוך* מקור. המקור כותב לכאן לאן
    להמשיך, העבודה שומרת את זה, והבקשה הבאה מרימה מאותה נקודה.
    """
...
class CatalogSource:
    ...
    name = ""
    tier = "aftermarket"
    # האם ``lookup`` מקבל ``resume``/``deadline`` ויודע להיעצר באמצע.
    # מקור שמביא עמוד אחד אינו צריך את זה, ולכן ברירת המחדל היא לא.
...
# מה שלא נשלח למודל. ``select``/``option`` נוספו אחרי שנמדד: בעמוד
# קטלוג של PartSouq, פותח הטקסט המצומצם היה רשימת המטבעות המלאה
# ‏(USD, EUR, AUD, SAR, CAD, GBP, NZD, CLP, ZAR, AED, QAR, RUB…)
# ואחריה רשימת השפות. תפריט ניווט וכותרת תחתונה הם אותו סיפור. כל
# אלה חוזרים בכל עמוד באתר, ולכן הם *בהגדרה* לא מה שמבדיל בין העמוד
# הזה לאחר - ובכל זאת הם אכלו את תקציב ה-30K שאחריו הטקסט נחתך,
# כלומר דחקו החוצה בדיוק את רשימת החלקים שבשבילה באנו.
_SKIP_TAGS = {"script", "style", "noscript", "svg", "head",
              "select", "option", "nav", "footer"}
