...
def create_app(config_object=Config):
    ...
    from . import seed_catalog  # noqa: F401
    ...
    # ה-API פטור מאסימון ה-CSRF, כי לקוח שאינו דפדפן אינו יכול לקבל
    # אחד - אין לו טופס להביא אותו ממנו.
    #
    # מה שמגן עליו בפועל הוא SameSite=Lax על קוקי הסשן: דפדפן אינו
    # שולח אותו בבקשת POST/PUT/DELETE שיצאה מאתר אחר, ולכן דף זר לא
    # יכול לדבר עם ה-API בשם המשתמש. ההזדהות כאן היא אותו קוקי בדיוק
    # כמו במסכים - **אין מפתחות API במערכת** (ההערה שהייתה כאן טענה
    # שיש, ותיארה מנגנון שלא קיים).
    #
    # המשמעות: אם אי פעם יורד ה-SameSite, או שנוסף לקוח שאינו דפדפן,
    # צריך אימות משלו - לא להסתמך על מה שכתוב כאן.
    if app.config["CSRF_ENABLED"]:
        csrf.exempt(api_bp)
