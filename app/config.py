...
class Config:
    ...
    #
    # כבוי כברירת מחדל. הוא נולד דלוק בפרודקשן כשהאפליקציה עלתה בלי
    # שום מנגנון הרשאות, וזה כבר לא המצב: role_required שומר על כל
    # נקודת קצה שמשנה נתונים, ומכונאי נחסם בכולן. דלוק כברירת מחדל
    # פירושו שכל סביבה חדשה - staging, שירות שנבנה מחדש - נולדת
    # נעולה, ומי שמקים אותה מגלה שאי אפשר להזין מק"ט ומקבל הודעה
    # שמפנה אותו למערכת הרשאות שכבר עובדת.
    #
    # להדלקה בחירום: READ_ONLY=1
    READ_ONLY = os.environ.get("READ_ONLY", "0").strip().lower() in {
        "1", "true", "yes",
    }
